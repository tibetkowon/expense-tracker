...
Tasks 1–6 done and verified (scaffolding, Google sign-in, Sheets wrapper, Drive folder picker, manual entry, list/summary, visual design, receipt OCR). Task 6's live-photo manual verification (Step 8) and adding a real `AI_GATEWAY_API_KEY` to `.env.local` (Step 6) are still pending on the user. Tasks 7 (OCR-to-form integration), 8 (PWA installability) remain — see the plan doc's checkboxes for exact step-level status.

## Recent decisions

- 2026-09-03: Task 6 (receipt OCR) done. Re-verifying the plan's `ai` SDK snippets against the installed `ai@7.0.87` surfaced two deprecations since the plan was written: `generateObject` → `generateText` + `Output.object({ schema })` (`{ object }` result becomes `{ output }`), and the `{ type: 'image', image }` message content part → `{ type: 'file', mediaType, data }` (bare base64 string accepted directly as `data`, no data-URL prefix needed). Plan doc's Task 6 snippets were corrected in place before handoff to Codex. Model `google/gemini-3.5-flash-lite` confirmed still current on the AI Gateway. `AI_GATEWAY_API_KEY` and a real-photo test are still needed from the user before OCR can be exercised end-to-end.

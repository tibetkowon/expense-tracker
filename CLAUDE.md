...
- 2026-09-15: OCR provider plan changed again — Google AI Studio (the 2026-09-08 decision) turned out not to work for the user's use case: AI Studio's Gemini API now requires a Prepay balance, and standard GCP Cloud Billing credits (including the monthly Google Developer Program premium credit the user gets) cannot fund that prepay balance. Vertex AI uses Postpay billing against the Cloud Billing account directly, so it's the only path where the credit actually applies. Decided to switch to Vertex AI (`@ai-sdk/google-vertex`, service-account-key auth — Vercel's OIDC/Workload Identity Federation option was considered and rejected as too much GCP-console setup for a personal project). See `docs/superpowers/plans/2026-09-15-ocr-vertex-ai-switch.md`.
- 2026-09-07: Replaced the placeholder app icons (feedback item 5, scoped down — the fuller "PWA install prompt" ask was declined; iOS Safari doesn't support `beforeinstallprompt` anyway, see the 2026-09-06/07 entries' spike notes). `public/icons/icon-*.png` (PWA manifest icons) had shipped as a broken "missing glyph" fallback render, not a real design; replaced with a bold white ₩ mark on indigo-600. Also added `app/icon.png` (favicon) and `app/apple-icon.png` (what iOS Safari's Share → Add to Home Screen actually uses). Rendered via a real browser (Playwright) at each exact pixel size rather than Next's `next/og` `ImageResponse` — that path failed to fetch a fallback font for the ₩ glyph in this environment and would have shipped another blank icon.
- 2026-09-07: Payment method select box CSS fixed (feedback item 2) and Google Picker dialog given a Korean title (feedback item 3, scoped down after confirming Google's Picker has no CSS/theming API — cross-origin iframe from Google's own domain, only title/size/locale/hideTitleBar are controllable, and size/hideTitleBar were skipped as likely to hurt mobile layout more than help). Feedback item 3's fuller ask ("match the app's design") is not achievable without dropping the Picker for a custom Drive-API-backed folder browser, which would require broadening OAuth scope beyond `drive.file` (e.g. `drive.readonly`) — explicitly rejected to keep the minimal-scope constraint.
- 2026-09-07: `~/.claude/scripts/codex-auto.sh` (global, not part of this repo) had `--recount` added to its `git apply` calls after two separate codex-auto "fix round" patches failed with corrupt-patch errors — the model hand-writes unified diffs in its read-only sandbox and occasionally miscounts hunk header line counts; `--recount` makes git ignore those counts and recompute them from the actual +/- lines, which fixed the reproduced case. A rarer failure mode (missing `new file mode` line on new-file patches) isn't fixed by this and still needs manual intervention if it recurs.
- 2026-09-07: Edit/delete for individual expense rows shipped (user feedback item 1). Row identity is the spreadsheet row number within a month's sheet (no ID column). PATCH `/api/expenses` moves a row to a different month's sheet if the edited date's month changes; DELETE removes it via `deleteDimension`. Implemented by Codex via `codex-auto`; its review pass caught a real bug (edit/delete re-reading the storage-location picker's live localStorage value instead of the location that actually produced the displayed list, risking a cross-file mutation) — fixed by capturing and threading through the fetch-time folderId/fileName. Same live-account verification gap as the 2026-09-06 entry below (not yet done).
